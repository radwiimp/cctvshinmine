...
module.exports = function(s,config,lang){
    ...
    s.mergeDetectorBufferChunks = function(monitor,callback){
        ...
        var mergedFile = 'before_'+s.formattedTime()+'.mp4'
		...
		s.fileStats(allts,function(err,stats){
                    if(err){
                        ...
                        exec(cat,function(){			  
                            var merger = spawn(config.ffmpegDir,s.splitForFFPMEG(('-re -i '+allts+' -acodec copy -vcodec copy -ss 00:00:10 '+pathDir+mergedFile)))
			    //var merger = spawn(config.ffmpegDir,s.splitForFFPMEG(('-re -i '+allts+' -acodec copy -vcodec copy '+pathDir+mergedFile)))			
		            //var merger = spawn(config.ffmpegDir,s.splitForFFPMEG(('-re -i '+allts+' -acodec copy -vcodec copy -ss 00:00:' + videoLength + ' '+pathDir+mergedFile)))
    ...
    s.mergeDetectorBufferChunksAfter = function(monitor,savedFilename,callback){
        var pathDir = s.dir.streams+monitor.ke+'/'+monitor.id+'/'
        //var mergedFile = s.formattedTime()+'After.mp4'
	var mergedFile = 'after_'+savedFilename+'.mp4'
        var mergedFilepath = pathDir+mergedFile
        //s.systemLog('In mergeDetectorBufferChunksAfter : '+mergedFilepath)
        fs.readdir(pathDir,function(err,streamDirItems){
            var items = []
            var copiedItems = []
            var videoLength = s.group[monitor.ke].rawMonitorConfigurations[monitor.id].details.detector_send_video_length
            ...
            if(videoLength.length === 1)videoLength = '0' + videoLength
            var createMerged = function(copiedItems){
                var allts = pathDir+items.join('_')+'More'
                s.fileStats(allts,function(err,stats){
                    if(err){
                        //not exist
                        var cat = 'cat '+copiedItems.join(' ')+' > '+allts
            ...
            streamDirItems.forEach(function(filename){
                if(filename.indexOf('detectorStream') > -1 && filename.indexOf('.m3u8') === -1 && filename.indexOf('_') === -1 && filename.indexOf('m.ts') === -1){
                    items.push(filename)
                }
            })
            items.sort()
		 ...
		 //s.systemLog('Error filename read :' + pathDir+filename)
                }
            })
        })
    }
